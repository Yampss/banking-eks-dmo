...
REDIS_HOST=$(aws ssm get-parameter --name "/nexabank/production/redis-host" --region "${AWS_REGION}" --query 'Parameter.Value' --output text 2>/dev/null || echo "localhost")
...
BEDROCK_MODEL=$(aws ssm get-parameter --name "/nexabank/production/bedrock-model-id" --region "${AWS_REGION}" --query 'Parameter.Value' --output text 2>/dev/null || echo "anthropic.claude-3-haiku-20240307-v1:0")
